...
より汎用的な解決策として、「bockchain automation」という支援技術/サービスがあります。これは、ブロックチェーンの外側のネットワークにおいてbotを実行してブロックチェーンを定期的に監視し、スマートコントラクトの関数を適切なタイミングで実行する仕組みです。botの実態はlisten.jsのようなプログラムです。botの実行をインターネット上に分散的に行うことでbot自体が攻撃やトラブル等で停止しないような仕組みを備えているものが多いようです。このようなサービスを用いれば、自分でサーバーをたててlisten.jsを実行し続けなくても、スマートコントラクトの運用が安定して可能になります。
